...
class FilmDetailViewModel @Inject constructor(
    ...
    private fun updateUiState(updateUiModel: (oldUiModel: FilmDetailUiModel) -> FilmDetailUiModel) {
        _uiStateFlow.update { uiState ->
            when (uiState) {
                is FilmDetailUiState.Content -> FilmDetailUiState.Content(updateUiModel(uiState.filmDetailUiModel))
                else -> FilmDetailUiState.Content(updateUiModel(FilmDetailUiModel()))
            }
        }
    }
    ...
    private fun getStaff(id: Int) {
        getFilmInfo(request = {
            ...
            updateUiState { oldModel ->
                ...
                val workers = staff?.filter { it.profession != TypeStaff.ACTOR }
                oldModel.copy(
                    actor = actors,
                    staff = workers,
                    isActorsVisible = !actors.isNullOrEmpty() && !isError,
                    isStaffVisible = !workers.isNullOrEmpty() && !isError,
                )
            }
        })
    }
    ...
    private fun getGallery(id: Int) {
        getFilmInfo(request = {
            ...
            updateUiState { oldModel ->
                oldModel.copy(
                    gallery = gallery,
                    isGalleryVisible = !gallery.isNullOrEmpty() && !isError,
                )
            }
        })
    }
    ...
    private fun getSimilar(id: Int) {
        getFilmInfo(request = {
            ...
            updateUiState { oldModel ->
                oldModel.copy(
                    similar = similar,
                    isSimilarVisible = !similar.isNullOrEmpty() && !isError,
                )
            }
        })
    }
